...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const userId =
      request.headers.get("x-user-id") || body.user_id || getUserId(request);
...
export const DELETE: RequestHandler = async ({ url, request }) => {
  const mangaId = url.searchParams.get("manga_id");
  const userId =
    url.searchParams.get("user_id") || request.headers.get("x-user-id");

  if (!mangaId || !userId) {
    return json(
      { success: false, error: "Manga ID and user ID required" },
      { status: 400 },
    );
  }
