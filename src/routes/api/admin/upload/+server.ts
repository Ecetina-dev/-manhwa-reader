...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    console.log(
      `   Original: ${(originalSize / 1024).toFixed(1)}KB → Compressed: ${(compressed.size / 1024).toFixed(1)}KB`,
    );
    console.log(
      `   Saved: ${savedPercent}% (${(savedBytes / 1024).toFixed(1)}KB)`,
    );
    console.log(
      `   Dimensions: ${originalDimensions.width}x${originalDimensions.height} → ${compressed.width}x${compressed.height}`,
    );
