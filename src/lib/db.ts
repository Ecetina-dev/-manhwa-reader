...
export async function saveProgress(
  ...
  if (db) {
    ...
    const progress: Record<string, StoredReadingProgress> = stored ? JSON.parse(stored) : {};
 ...
 */
export async function getProgress(serieId: string): Promise<StoredReadingProgress | undefined> {
  const db = await openManhwaDB();
