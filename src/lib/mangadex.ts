...
export async function getMangaList(offset = 0, limit = 20): Promise<Serie[]> {
  try {
    ...
    const response = await throttledFetch(`${MANGADEX_API}/manga?${params.toString()}`);
...
export async function getMangaById(id: string): Promise<Serie> {
  ...
  const response = await throttledFetch(`${MANGADEX_API}/manga/${id}?${params.toString()}`);
...
export async function getChapters(mangaId: string): Promise<Chapter[]> {
  ...
  try {
    ...
    const response = await throttledFetch(`${MANGADEX_API}/manga/${mangaId}/feed?${params.toString()}`);
...
export async function getChapterPages(chapterId: string, mangaId?: string): Promise<MangaDexPage[]> {
  // Try cache first
  try {
    const cached = await cacheImageService.getCachedChapters()
  ...
  // Fetch from API
  const response = await throttledFetch(`${MANGADEX_API}/at-home/server/${chapterId}`);
  ...
  if (mangaId && pages.length) {
    try {
      await cacheImageService.markChapterCached(chapterId, mangaId, pages.map(p => p.url));
    } catch (e) {
