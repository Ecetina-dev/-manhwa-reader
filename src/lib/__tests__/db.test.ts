...
describe('Database Service (db.ts)', () => {
	...
	describe('getAllProgress', () => {
		it('should get all progress from IndexedDB', async () => {
			const mockProgress = [
				{ serieId: 'serie-1', chapterId: 'chapter-1', page: 5, updatedAt: Date.now() },
				{ serieId: 'serie-2', chapterId: 'chapter-2', page: 10, updatedAt: Date.now() },
			];
	...
	describe('getAllCachedChapters', () => {
		it('should get all cached chapters from IndexedDB', async () => {
			const mockChapters = [
				{ chapterId: 'chapter-1', mangaId: 'manga-1', cachedAt: Date.now(), pages: [] },
				{ chapterId: 'chapter-2', mangaId: 'manga-1', cachedAt: Date.now(), pages: [] },
			];
	...
	describe('deleteCachedChapter', () => {
		it('should delete cached chapter from IndexedDB', async () => {
			...
			expect(mockDB.delete).toHaveBeenCalledWith('cached-chapters', 'chapter-1');
		});
	});
	...
	describe('clearAllProgress', () => {
		...
		it('should fallback to localStorage when IndexedDB is null', async () => {
			...
			expect(localStorageMock.removeItem).toHaveBeenCalledWith('manhwa-reader-fallback');
		});
	});
});
