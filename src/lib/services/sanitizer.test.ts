...
describe('Sanitizer Service', () => {
  describe('stripHtml', () => {
    ...
    it('should handle nested tags', () => {
      expect(stripHtml('<div><p>Nested <span>text</span></p></div>')).toBe('Nested text');
    });
  });
  ...
  describe('sanitizeFilename', () => {
    ...
    it('should keep valid characters', () => {
      expect(sanitizeFilename('manga-cover-123.jpg')).toBe('manga-cover-123.jpg');
    });
  ...
  describe('sanitizeSearchQuery', () => {
    it('should trim and limit length', () => {
      ...
      expect(sanitizeSearchQuery('a'.repeat(200)).length).toBeLessThanOrEqual(100);
    });
  ...
  describe('sanitizeMangaContent', () => {
    it('should remove scripts and event handlers', () => {
      const input = '<p>Description</p><script>evil()</script><img onerror="alert(1)">';
