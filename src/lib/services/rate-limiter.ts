...
export function getEndpointType(url: string): keyof typeof CONFIGS {
  ...
  if (url.includes('/upload') || url.includes('/api/admin/') && url.includes('/pages')) {
    return 'upload';
  }
  if (url.includes('/api/') && (url.includes('/search') || url.includes('/browse'))) {
    return 'search';
  }
 ...
 */
export function rateLimitRequest(request: Request): { success: boolean; error?: string } {
