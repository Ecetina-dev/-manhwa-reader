...
class ErrorTrackerService {
  ...
  captureMessage(
    ...
    if (sentry && "captureMessage" in sentry) {
      (
        sentry as { captureMessage: (message: string, level?: string) => void }
  ...
  setUser(userId: string, email?: string): void {
    ...
    if (sentry && "setUser" in sentry) {
      (
        sentry as { setUser: (user: { id: string; email?: string }) => void }
      ).setUser({
        id: userId,
        email,
      });
  ...
  setTag(key: string, value: string): void {
    ...
    if (sentry && "setTag" in sentry) {
      (sentry as { setTag: (key: string, value: string) => void }).setTag(
        key,
        value,
      );
    }
