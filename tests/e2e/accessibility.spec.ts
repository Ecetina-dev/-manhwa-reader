...
test.describe('Accessibility', () => {
  ...
  test('images should have alt text or aria-label', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 5); i++) {
      ...
      expect(alt || ariaLabel || await img.evaluate(el => el.getAttribute('aria-hidden') === 'true')).toBeTruthy();
    }
  });
  ...
  test('form inputs should have labels', async ({ page }) => {
    ...
    expect(ariaLabel || labelledBy || page.locator('label[for="' + await searchInput.getAttribute('id') + '"]')).toBeTruthy();
  });
